...
int pa, parent;
int finde(int node , int par , vector<vector<int>>& adj , vector<int>& deg , int flag){
    if(deg[node]>2)
    {
        if(flag){ pa = node ;parent = par ;}
     return 1;}
    
    for(auto it : adj[node]){
        if(it!= par){
           return 1+finde(it , node , adj , deg , flag);
        }
    }
   return 1;
}
int main() {
   ...
   // vector<vector<long long>> test_cases(t);
    while(t--){
        ...
        int dep = finde(1 , 0 , adj , deg , 1);
        if(dep == n){cout << power2(dep) << endl;continue;}
        int dep1 =0, dep2 =0 ;
     
        for(auto it: adj[pa]){
            if(it!=parent){
                if(dep1==0){dep1 = finde(it , pa , adj , deg,0);}
                else{
                    dep2 = finde(it , pa , adj , deg,0);
                }
            }
        }
        if(dep1==dep2){cout << (2*power2(dep))%1000000007 << endl;}
        else{
            cout << ((power2(abs(dep1-dep2))+power2(abs(dep1-dep2)-1))*power2(dep))%1000000007 << endl;
        }
    }
}
