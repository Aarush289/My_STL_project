...
const ll MAX = 1e6 + 5;         // max n for precomputation
...
long double comb_ratio(ll A, ll B, ll k) {
...
long double nCr_log(ll n, ll k) {
    if(k < 0 || k > n) return -1e18; // log(0) ~ -inf
    return lgamma(n + 1.0) - lgamma(k + 1.0) - lgamma(n - k + 1.0);
}
// Compute nCr using logs
// long double nCr_log(ll n, ll r) {
//     if (r < 0 || r > n) return 0.0L;
//     r = min(r, n - r); // symmetry

//     long double log_res = 0.0L;
//     for (ll i = 1; i <= r; ++i) {
//         log_res += logl(n - i + 1) - logl(i); // ln(n-i+1) - ln(i)
//     }

//     return expl(log_res); // e^(log(nCr))
// }
ll nCr_mod_small(ll n, ll r, ll mod) {
    if(r < 0 || r > n) return 0;
    if(r == 0 || r == n) return 1;
    long long res = 1;
    for(long long i = 1; i <= r; ++i) {
        res = res * (n - i + 1) % mod;
        res = res * power(i, mod - 2, mod) % mod; // modular inverse
    }
    return res;
}

ll nCr_large(ll n, ll r, ll mod=MOD) {
    if(r == 0) return 1;
    return nCr_large(n / mod, r / mod, mod) * nCr_mod_small(n % mod, r % mod, mod) % mod;
}
...
// do sieve() for generating bool vector of whether number is prime or not upto 2*1e7
// ignore my take functions
// prie(n) checks whether n is prime or not in root(n) time
// power2(n) everything long long , and taken MOD with 1e9+7
// power ( a,b) a^b but no mod is used here
// power(a,b,mod)   
// init_factorial() precalculates factorials upto 1e6 
// nCr(a,b) by default mod is 1e9+7
// nPr(a,b) by default mod is 1e9+7
// nCr_log (a,b) returns long double and use log to prevent overflow but have to be used as following 
// Compute probability using logs to avoid overflow 
//double log_num = nCr_log(sum - v[h-1], n - 1); 
//double log_den = nCr_log(sum - 1, n - 1); 
//double ans = 1.0 - exp(log_num - log_den); // safe subtraction in log space
// nCr_large(a,b) if somehow we need nCr for numbers more than 1e6 (out of bound of precompute thing)
// prime_factor(n) return vector of prime factors
// gcd(a,b) 
// lcm(a,b)
// get_primes(n) return primes upto n in vector
// isPerfectSquare(n) 
// min_penalty(y,b,c) returns minimum x>=y such that all bits of b|c are on in x 
// editdistance(takes 2 strings ) minimum no of steps needed to make A=B (delete , add , change
// longest_common_subsequence( vector A , vector B)
// knapsack01(vector weight , vector value , vector target)
// unboundedknapsack(same as above)
// countsubsets(arr , target) return number of subsets with target sum
// LDS //LIS longest increasing or decreasing subsequence
// mod_inverse(a , mod) where mod is by default 1e9+7 so mod_inverse(a) will by default consider mod
