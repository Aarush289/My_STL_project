...
then from last digit go upto first digit 
bucket[10] then find index of each which is element at that pos;
then make cumulative distribution 
then make result array for each element result[count[arr[i]/pos)%10]-1]=arr[i];

for strings , just change bucket size to 27 and index is calculated as (pos<arr[i].size) ?arr[i][pos]-'a'+1: 0
*/

//////BUCKET-SORT//////
/*
Make whatever size of bucket u wanna use then find range as max-min+1/count ; then for each elemet find index as 
element-min/range then sort each bucket and put bck to array 
*/