...
//map<ll,ll> m;
        ...
        if(!sum){cout << "Yes" << endl;continue;}
       
        //if(m[sum]>=2){cout << "Yes" << endl;continue;}
        // cout << "No" << endl;
   ...
   //    ll sum1=0,sum2=0,sum3=0;
    ...
    ll flag=0;
    // Step 2: Try all i and j (0 ≤ i < j < n)
    for (int i = 0; i < n - 2 && !flag ; ++i) {
        for (int j = i + 1; j < n - 1 && !flag; ++j) {
            ll xor1 = prefix_xor[i];                                // 0 to i
            ll xor2 = prefix_xor[j] ^ prefix_xor[i];                // i+1 to j
            ll xor3 = prefix_xor[n - 1] ^ prefix_xor[j];            // j+1 to n-1

            if (xor1 == xor2 && xor2 == xor3) {
               // cout << "Equal XOR found at i=" << i << ", j=" << j << " with XOR = " << xor1 << endl;
               flag=1;break;
            }
        }
        if(flag){break;}
