...
double sqrt_precise(double x) {
    double low = 0, high = x;
    if (x < 1) high = 1; // for x in (0, 1)
    double mid;
...
void collect_sums(vector<int>& a, int l, int r, unordered_set<long long>& sums) {
    if (l == r) {
        sums.insert(a[l]);
        return;
    }
    ...
    collect_sums(a, l, mid, sums);
    collect_sums(a, mid + 1, r, sums);

    // Collect cross subarray sums (from mid to mid+1)
    vector<long long> left_sums, right_sums;

    long long temp = 0;
    for (int i = mid; i >= l; --i) {
        temp += a[i];
        left_sums.push_back(temp);
    }

    temp = 0;
    for (int i = mid + 1; i <= r; ++i) {
        temp += a[i];
        right_sums.push_back(temp);
    }

    for (auto l_sum : left_sums) {
        for (auto r_sum : right_sums) {
            sums.insert(l_sum + r_sum);
        }
    }
}

unordered_set<long long> all_subarray_sums(vector<int>& a) {
    unordered_set<long long> sums;
    collect_sums(a, 0, a.size() - 1, sums);
    return sums;
}

// void print(int arrcopy[], int n)
// {
//     for (int i = 0; i < n; i++)
//     {
//         int val = mp[arrcopy[i]].front();
//         mp[arrcopy[i]].pop();
//         cout << val << " ";
...
int main()
{
    ...
    while (cases--)
    {
       ...
       int count1=0;
         ...
         if(count1==n){while(n--){cout << "0" << " ";}cout << endl;continue;}
       for(int i = n-1; i >=0; i--)
       {   
            if(i==0){cout << "-1" ;break;}
            if(str[i]=='0'){
                cout << count << " ";if(j==i){j--;}continue;
            }
            count+=abs(i-j);
            while(j>=0 && str[j]!='0' ){
                j--;count++;
