...
long long int power(ll base2,long long int m)
{
    long long result = 1;
    ll base = base2;
    while (m > 0)
...
void removeCommonFactors(ll &a, ll &b) {
    ll g = __gcd(a, b); // Get GCD of a and b

    // Remove all occurrences of g from both a and b
    while (__gcd(a, b) != 1) {
        g = __gcd(a, b);
        a /= g;
        b /= g;
...
int main() {
    ...
    while(cases--){
        ...
        ll div = (n)*(n-1)/2;
        removeCommonFactors(div,ans);
       // cout << ans << " " << div << endl;
        ll div2 = power(div,1000000005);
         ans = ((ans%(1000000007))*(div2))%(1000000007);
        cout << ans << endl;
