...
int calcul(vector<long long>& dp, map<int,int>& m, int n , int limit) {
    if (n == 0) return 1;
    if (n < 0) return 0;   // better to return 0 (no way) instead of -1

    if (dp[n] != -1) return dp[n];

    long long ways = 0;
    for (auto it = m.rbegin(); it != m.rend(); ++it) {
     if(it->first<=limit) {ways += calcul(dp, m, n - it->first , it->first);}
    }
    ways%=(1000000007);
    return dp[n] = ways;
}

const long long MOD4 = 1000000007;
ll minim = -1e17;
ll cal(vector<vector<ll>>& dp , ll curr , ll max , ll total) {
    if(total==0){return 1;}
    if(total<0){return 0;}
   ...
   return dp[curr][total] = (cal(dp , curr+1 ,max , total ) + cal(dp , curr+1 ,max , total-curr))%MOD4;
}

ll cal(vector<int>& dp ,vector<pair<int,int>>& v , map<pair<int,int>, int>& m , int last , int index){
    if(dp[last]!=-1){return dp[last];}
    if(index >= v.size()){return dp[last]=0;}
    int flag=0;
    int i;
    for(i=index ;i< v.size();i++){
        if(v[i].first >= last){flag=1;break;}
    }
    if(!flag){return dp[last]=0;}
    return dp[last] = max(m[v[i]]+ cal ( dp , v , m , v[i].second , index+1) , cal(dp , v, m, last , index+1)) ;
}

int main() {
    long double ans, r, R, l;
    cin >> R >> l;
    long double v = R*(R+l);
    r = sqrtl(v);
    cout << setprecision(15) << (long double)0.05 - (r - R) << "\n";
}