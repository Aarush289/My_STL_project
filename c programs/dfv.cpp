...
int main() {
    ...
    while(cases--){
        ...
        max--;
        while(!(in1<0 && in2 >= n)){
            while(in1 >=0 && (in2>=n || arr[in1] < arr[in2])){
               if(arr[in1]>max){v.push_back(arr[in1]-max+temp);}
               in1--;max--;
            }
            while(in2 < n && (in1<0 || arr[in2] < arr[in1])){
                if(arr[in2]>max){v.push_back(arr[in2]-max+temp);}
                in2++;max--;
            }
        }
        sort(v.begin(), v.end());
